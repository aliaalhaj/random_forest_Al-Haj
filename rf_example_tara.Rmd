...
```{r}
require(tidyverse)
otutable_raw <- read_csv(file = "https://github.com/aliaalhaj/random_forest_Al-Haj/blob/master/otu.csv")
...
taxatable_raw <- read_csv(file = "https://github.com/aliaalhaj/random_forest_Al-Haj/blob/master/taxa.csv")
...
map <- read_csv(file = "https://github.com/aliaalhaj/random_forest_Al-Haj/blob/master/Tara_oceans_mapping_file.csv")
...
#Summary of analysis
MIX looks a lot like DCM and SRF. This is likely due to low sample size from MIX. Use the code below to see what the random forest analysis looks like with MIX removed. 

If you'd like to compare abundance across depths for the top 20 taxa, please see Abundant Taxa Bar Plot.


#Re-do analysis without MIX layer
Use original datafiles from first chunk:
```{r}

require(tidyverse)
require(phyloseq)
otu.m <- as.data.frame(otutable_raw)
otu.m<-subset(otu.m, select=-c(X1)) #if there are more columns that are non-numeric (i.e. taxonomy) add in to the -c(). For example, if you have a column named taxonomy select=-c(X1, taxonomy).
row.names(otu.m) <- otutable_raw$X1

#add depth to the file
#upload mapping file

#what tara samples have MIX as depth?
MIX <- subset(map, Depth=="MIX")
MIX

#Remove columns with the sample IDs identified in MIX
otu.m <- otu.m[ ,-c(58,60,62,95)]
map.m <- map[-c(58,60,62,95),]

otu.m <- otu_table(otu.m, taxa_are_rows = T)

names(map.m)[1] <- 'SampleID'
row.names(map.m) <- map.m$SampleID
map.m[2:4] <- lapply(map.m[2:4] , factor) #change to columns where your factors are
map2.m <- data.frame(map.m)
meta.m <- sample_data(map.m)

## import as phyloseq object (If you already have a phyloseq object, you can start from here.)
ps.m <- phyloseq(otu.m, taxonomy, meta.m)
...
```{r}
## Remove taxa not seen more than 3 times in at least 20% of the samples 
ps2.m = filter_taxa(ps.m, function(x) sum(x > 3) > (0.2*length(x)), TRUE)
...
## filter original `otu` object to retain only those taxa that pass filter
otu2.m <- otu.m[taxa_names(ps2.m),]   ## this is the object we'll use for RF classifying
```
...
Transforming our ASV table and metadata into a single object. First, transpose the `otu` table, then append whichever metadata you want to run the classifier with. In this example, we'll use the metadata column `$Depth` - the depth the samples were obtained:
```{r}
t_otu.m <- data.frame(t(otu2.m))  ## update the ASV table of interest given filtering cosiderations above
t_otu.m$Depth <- map2.m[rownames(t_otu.m), "Depth"]
```
...
require(randomForest)
set.seed(151)
RF_state_classify <- randomForest( x=t_otu.m[,1:(ncol(t_otu.m)-1)] , 
                                   y=t_otu.m[ , ncol(t_otu.m)] , 
                                   ntree=501, importance=TRUE, proximities=TRUE )
```
...
require(tidyverse)
require(ggplot2)
cfuzmat.m <- data.frame(RF_state_classify$confusion)
cfuzmat.m$Predicted <- row.names(cfuzmat.m)
tmp.m <- cfuzmat.m
tmp.m$class.error <- NULL
tmp.m$Predicted <- NULL
tmp2.m <- sweep(tmp.m, 1, rowSums(tmp.m), '/')
tmp2.m$class.error <- tmp2.m$class.error
tmp2.m$Predicted <- row.names(tmp2.m)
cfuzplot.m <- gather(data = tmp2.m, key = Expected, value = value, c(1:3)) #change c(1:4) to the column numbers you would like to use
cfuzplot.m$Expected <- gsub("^X", "", cfuzplot.m$Expected)  ## removing the X from label name...
hm.2 <- ggplot(cfuzplot.m, aes(x=Predicted, y=Expected)) +
  geom_tile(aes(fill=value), color="grey50") +
  scale_fill_viridis_c() +
  labs(fill="fraction of\nsamples assigned")
hm.2
...
require(rfUtilities)
RF_state_classify_sig <- rf.significance( x=RF_state_classify ,  
                                          xdata=t_otu.m[,1:(ncol(t_otu.m)-1)] , 
                                          nperm=100 , ntree=501 )

RF_state_classify_sig
```

Note that the model OOB error rate remains around the same 14.8% we observed in a single run.
...
require(caret)
fit_control <- trainControl( method = "LOOCV" )   ## method is "Leave One Out Cross Validation"
RF_state_classify_loo <- train( t_otu.m[,1:(ncol(t_otu.m)-1)] ,
                                 y=t_otu.m[, ncol(t_otu.m)] ,
                                 method="rf", ntree=501 , tuneGrid=data.frame( mtry=25 ) ,
                                 trControl=fit_control )
```
...
train_control <- trainControl(method="repeatedcv", number=5, repeats=10)
# train the model
model <- train(Depth ~ ., data = t_otu.m, trControl=train_control, method="rf", importance=TRUE)

# summarize results
print(model)
```

Obtaining the "importance" for each ASV to the model
```{r}
model_imp <- data.frame(varImp(model)$importance)
model_imp$taxa <- row.names(model_imp)
mi_plotdat <- gather(data = model_imp, 
                     key = State, 
                     value = Importance,
                     c(1:3)) #change c(1:3) for the number of columns of categories you have in your tmp file
```
...
```
#Summary:
The model did a good job of predicting the distinct groups.
